...
function showPlayer(){
  ...
  const marchEdit=document.getElementById('marchTimeEditInput');
  if(marchEdit)marchEdit.value=currentMarchTime||'00:00';

  const marchStatus=document.getElementById('marchTimeStatus');
  if(marchStatus&&currentMarchTime){
    marchStatus.textContent=`Current March Time: ${currentMarchTime}`;
  }
  ...
  if(offsetStatus){
    offsetStatus.textContent=`Current Offset: ${formatSignedOffset(getPlayerSendOffsetMs())}`;
  }
...
function listenToMyPlayer(){
  ...
  playerUnsub=onSnapshot(doc(db,'players',name),snap=>{
    ...
    if(d.marchTime&&d.marchTime!==currentMarchTime){
      ...
      const marchEdit=document.getElementById('marchTimeEditInput');
      if(marchEdit)marchEdit.value=currentMarchTime;

      const marchStatus=document.getElementById('marchTimeStatus');
      if(marchStatus)marchStatus.textContent=`Current March Time: ${currentMarchTime}`;
    }
...
window.loginPlayer=async()=>{
  ...
  if(parseMmSsToSeconds(marchTime)===null){
  ...
  await setDoc(
    doc(db,'players',name),
    {name,marchTime,updatedAt:serverTimestamp()},
    {merge:true}
  );
...
window.updateMarchTime=async()=>{
  ...
  await setDoc(
    doc(db,'players',name),
    {name,marchTime:value,updatedAt:serverTimestamp()},
    {merge:true}
  );
  ...
  // March time is part of the real send calculation, so active rallies recalculate.
  resetSendStampCache();
  renderRallies();
};
...
window.adjustSendOffset=(deltaSeconds)=>{
  ...
  if(status){
    status.textContent=`Current Offset: ${formatSignedOffset(getPlayerSendOffsetMs())}`;
  }
};
...
function resetSendStampCache(){
  Object.keys(localStorage).forEach(k=>{
    if(k.startsWith(`sendStamp_${linkedName()}_`)){
    ...
    .map(r=>{
      ...
      return `<div class="rally-row player-phase-row" style="color:${color}">
        ...
        <div class="player-timer-pair">
          <div class="player-timer-block">
            <div class="player-timer-label">Send</div>
            <div class="rally-countdown ${sendCls}" ${sendColor}>${countdownText(r.timing.sendSeconds)}</div>
          </div>
          <div class="player-timer-block">
            <div class="player-timer-label">March</div>
            <div class="rally-countdown" style="color:${color}">${countdownText(marchDisplay)}</div>
          </div>
        </div>
...
window.addEventListener('load',()=>{
  ...
  currentMarchTime=m||'';
  draftSendOffsetMs=getPlayerSendOffsetMs();

  const offsetInput=document.getElementById('sendOffsetInput');
  if(offsetInput)offsetInput.value=formatSignedOffset(draftSendOffsetMs);
