import {db,doc,setDoc,deleteDoc,onSnapshot,collection,getDocs,serverTimestamp,autoFormatMmSs,setupUI,setThemeFromSwitch,ADMIN_PIN,RALLY_BASE_SECONDS,parseMmSsToSeconds,makeId,colorForId,countdownClass,countdownText,getRallyTimerSeconds,isExpiredRally,esc} from './shared.js';
...
window.createActiveRally=async(id)=>{
  ...
  await setDoc(doc(db,'activeRallies',rallyId),{
    ...
    color,
    rallyTimerEndMs:now+(RALLY_BASE_SECONDS*1000),
    createdMs:now,
    sent:false,
...
window.adjustActiveRally=async(id,delta)=>{
  ...
  await setDoc(doc(db,'activeRallies',id),{
    rallyTimerEndMs:Number(r.rallyTimerEndMs||Date.now())+(Number(delta)*1000),
    updatedAt:serverTimestamp()
  },{merge:true});
};
...
function renderActiveRallies(){
  ...
  const rows=Object.entries(activeRallies)
    ...
    .sort((a,b)=>Number(a.rallyTimerEndMs)-Number(b.rallyTimerEndMs))
    .map(r=>{
      const sec=getRallyTimerSeconds(r,now);
      ...
      return `<div class="active-instance" style="color:${color}">
        <div class="instance-head">
          <div>
            ...
            <div class="instance-meta">Visible rally timer</div>
