import {db,doc,setDoc,deleteDoc,onSnapshot,collection,serverTimestamp,autoFormatMmSs,setupUI,setThemeFromSwitch,ADMIN_PIN,RALLY_BASE_SECONDS,parseMmSsToSeconds,makeId,colorForId,countdownClass,countdownText,getRallyCountdownSeconds,isExpiredRally,esc} from './shared.js';
...
window.createActiveRally=async(id)=>{
  ...
  await setDoc(doc(db,'activeRallies',rallyId),{
    ...
    color,
    rallyEndMs:now+(RALLY_BASE_SECONDS*1000),
    createdMs:now,
    sent:false,
...
window.adjustActiveRally=async(id,delta)=>{
  ...
  if(!r)return;
  await setDoc(doc(db,'activeRallies',id),{
    rallyEndMs:Number(r.rallyEndMs||Date.now())+(Number(delta)*1000),
...
window.sendToPlayers=async(id)=>{
  ...
  // Enemy reaches castle after the visible rally timer ends, then the enemy march completes.
...
function renderActiveRallies(){
  ...
  const rows=Object.entries(activeRallies)
    .map(([id,r])=>({id,...r}))
    .filter(r=>!isExpiredRally(r,now))
    .sort((a,b)=>Number(a.rallyEndMs)-Number(b.rallyEndMs))
    .map(r=>{
      const sec=getRallyCountdownSeconds(r,now);
      const color=r.color||colorForId(r.profileId||r.id);
      const cls=countdownClass(sec);
      ...
      return `<div class="active-instance" style="color:${color}">
        <div class="instance-head">
          <div>
            ...
            <div class="instance-meta">Visible in-game rally timer</div>
            ${r.sent?'<span class="badge">Sent to players</span>':''}
          </div>
          <div class="instance-timer ${cls}" ${timerColor}>${countdownText(sec)}</div>
        </div>
        <div class="instance-controls">
          <button class="secondary" onclick="adjustActiveRally('${r.id}',-1)">-1</button>
          <button class="secondary" onclick="adjustActiveRally('${r.id}',1)">+1</button>
...
async function autoDeleteExpired(){
  ...
  try{
    ...
    Object.entries(activeRallies).forEach(([id,r])=>{
      if(isExpiredRally(r,now))deletes.push(deleteDoc(doc(db,'activeRallies',id)));
    });

    Object.entries(sentRallies).forEach(([id,r])=>{
      if(Number(r.enemyHitEndMs)&&now>=Number(r.enemyHitEndMs)+2000){
        deletes.push(deleteDoc(doc(db,'sentRallies',id)));
      }
    });
...
onSnapshot(collection(db,'rallyProfiles'),snap=>{
  profiles={};
  snap.forEach(d=>profiles[d.id]=d.data());
  renderProfiles();
});

onSnapshot(collection(db,'activeRallies'),snap=>{
  activeRallies={};
  snap.forEach(d=>activeRallies[d.id]=d.data());
  renderActiveRallies();
});
